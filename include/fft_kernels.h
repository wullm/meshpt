...
static inline void kernel_sqrt_power_spline(struct kernel *the_kernel) {
    ...
    if (k == 0) {
        ...
        const struct power_spline *spline = (const struct power_spline *) the_kernel->params;

        /* Find the k index and spacing for this wavevector */
        int k_index;
        double u_k;
        powerSplineFindK(spline, k, &k_index, &u_k);

        /* Evaluate the transfer function for this tau and k */
        the_kernel->kern = sqrtPowerSplineInterp(spline, k_index, u_k);
    }
}
