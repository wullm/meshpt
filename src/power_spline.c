...
int init_power_spline(struct power_spline *spline, int k_acc_size) {
...
/* Clean the memory */
int free_power_spline(struct power_spline *spline) {
...
int power_spline_find_k(const struct power_spline *spline, double k, int *index,
                        double *u) {
    ...
    int start =
        spline->k_acc_table[J < k_acc_table_size ? J : k_acc_table_size - 1];

    /* Search in the k vector */
    int i;
    for (i = start; i < k_size; i++) {
        if (k >= spline->k[i] && k <= spline->k[i + 1])
            break;
    }
...
double power_spline_interp(const struct power_spline *spline, int k_index,
                           double u_k) {
...
double sqrt_power_spline_interp(const struct power_spline *spline, int k_index,
                                double u_k) {
...
//     return perturbSplineInterp(spline, k_index, tau_index, u_k, u_tau,
//     index_src);
// }
